...
const BookingScreen = () => {
  ...
  const showButtonsBorders = (showAddSelection || showPaymentBtmBox || showCancelBtmButton || showNoAvailableCarsBtmBox);
  ...
  const handleTopLeftIconClick = () => {
    if (showPaymentBtmBox) {
      ...
      //all the cases for if (!showAddressBottomSheet && !isAddLocationClicked && !showPaymentBtmBox && !showStopsBtmBox && !showAddSelection && !showCancelBtmButton)
      //@todo - implement drawer open part
  ...
  const handleMultibleBtmBoxHeight = () => {
    ...
    } else if (showDriverArriving || showDriverArrived || showOnTrip) {
      return 240;
    } else {
